<div id="add-device-popup" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white p-6 rounded-[20px] shadow-lg w-full max-w-md p-[20px]">
        ...
        <form method="POST" action="{{ route('devices.storeDevice') }}">
            ...
            <input type="text" name="name" id="name" placeholder="Device Name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
        </div>
        <div class="mb-4">
            <label for="device" class="block text-sm font-medium text-gray-700">Device Number</label>
            <input type="text" name="device" id="device" placeholder="Device Number" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
        </div>
